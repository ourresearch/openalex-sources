...
Runs in one continuous pass (~137 pages, ~5 min on a Heroku dyno). On failure,
re-run from scratch — Crossref's deep-pagination cursors expire quickly and
one-off dyno storage is ephemeral, so there is deliberately no resume mechanism.
