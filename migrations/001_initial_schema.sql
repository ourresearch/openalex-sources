-- 001 initial sources schema (oxjob #548)
...
-- the sources registry (PK = S-id, preserved / minted from source_id_seq)
...
CREATE INDEX IF NOT EXISTS idx_sources_publisher_id ON sources(publisher_id);
CREATE INDEX IF NOT EXISTS idx_sources_endpoint_id ON sources(endpoint_id);
CREATE INDEX IF NOT EXISTS idx_sources_merge_into_id ON sources(merge_into_id);
CREATE INDEX IF NOT EXISTS idx_sources_active ON sources(id) WHERE merge_into_id IS NULL;

-- normalized ISSN membership; UNIQUE(issn) = one-ISSN-one-source invariant
CREATE TABLE IF NOT EXISTS source_issn (
...
CREATE TABLE IF NOT EXISTS issn_to_issnl (
    issn         TEXT PRIMARY KEY,
    issn_l       TEXT,
    note         TEXT,
    updated_date TIMESTAMPTZ DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_issn_to_issnl_issn_l ON issn_to_issnl(issn_l);

-- go-forward S-id minting sequence (loader resets to MAX(id) after backfill)
CREATE SEQUENCE IF NOT EXISTS source_id_seq START WITH 7407059452;

-- Databricks-facing export view (the federation read contract): restores the
-- flat issn/issns/webpage shape of openalex.sources.sources.
CREATE OR REPLACE VIEW source_export AS
SELECT
    s.id,
    s.display_name,
    s.type,
